...
public class ChatRoomController {
	...
	public List<Message> returnDatabase(@PathVariable("chatRoomId") String chatRoomId){
		return chatRoomService.getActiveRoom().getDatabase().getMessages(); //A partir de un id de sala dado, devuelve una lista con los mensajes guardados en la base de datos
	}
	
	@PostMapping("{chatRoomId}") //Post de mensaje
	public ResponseEntity<Boolean> putAnuncio(@RequestBody Message message, @PathVariable("chatRoomId") String chatRoomId){ //A paritr de un mensaje y el id de una sala...
		chatRoomService.getActiveRoom().getDatabase().setMessage(message); //...añade el mensaje a la base de datos de la sala activa
		return new ResponseEntity<Boolean>(true, HttpStatus.CREATED); //Devuelve que el Http esta creado
	}
